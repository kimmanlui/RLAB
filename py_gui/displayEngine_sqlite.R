...
if (!exists("/kim/gitdir/RLAB/py_gui/main_ENV_sqlite.R"))
{
  print("run env_sqlite")
  source("/kim/gitdir/RLAB/py_gui/main_ENV_sqlite.R")
}
  ...
  if (dweek)
  {
    while (firsttime==1 || startTime<=istamp && istamp<endTime)
    {
      if ( firsttime==1 || (startTime<=istamp && istamp<=startLunch) ||  (endLunch<=istamp && istamp<endTime) )
      {
          ...
          source("/kim/gitdir/RLAB/py_gui/main_sqlite.R")
